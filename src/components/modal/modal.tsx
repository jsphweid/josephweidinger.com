...
export default class Modal extends React.Component<ModalProps> {
  ...
  renderCloseButton = (): JSX.Element => {
    return (
      <div onClick={this.props.closeModal} className="jlw-modal-closeButton">
        <div>
          <div />
        </div>
      </div>
  ...
  maybeRenderImage = (): JSX.Element => {
    ...
    const imgSrc: string = imgUrl.replace(
      "---cloudinaryStyleString---",
      cloudinaryStylePortion
    );
    return (
      <img
        src={imgSrc}
  ...
  render() {
    ...
    return (
      <div className={`jlw-modal ${this.props.className || ""}`}>
        ...
        <div className="jlw-modal-content">
          {this.props.project.getModalContent()}
        </div>
        <div className="jlw-modal-date">
          Date: <strong>{date}</strong>
        </div>
        <button className="jlw-modal-button" onClick={this.props.closeModal}>
          Close
        </button>
      </div>
    );
